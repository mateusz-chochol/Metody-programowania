...
std::fstream createFileStream(const std::ios_base::openmode streamType)
{
	if (streamType == std::ios_base::in)
	{
		...
		exitProgram("Function \"createFileStream()\" couldn't find a file with a given name.");
	}
	...
	exitProgram("Function \"createFileStream()\" cannot create filestream in modes other than in or out.");
}
...
bool checkFileExtension(const std::string& fileName)
{
	if (fileName.length() < 4)
		exitProgram("Function \"checkFileExtension()\" was given a file with too short name (less than 4 characters)");
