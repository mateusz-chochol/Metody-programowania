...
int getNumberFromFile(std::fstream &file)
{
	...
	while (numberAsString.length() == 0 || letterIndex < numberAsString.length())
	{
		if (numberAsString.length() == 0 || ((numberAsString[letterIndex] < '0' || numberAsString[letterIndex] > '9') && numberAsString[letterIndex] != '-'))
			exitProgram();
		else
			letterIndex++;
	}
